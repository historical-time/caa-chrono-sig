neonet <- read.csv(paste0(file = "C:/Users/nicco/Downloads/NEONET/140_id00140_doc_elencoc14.csv"),
                   header = T,
                   sep = ";")

#load Binder file
binder <- read.csv(paste0(file = "C:/Users/nicco/Downloads/NEONET/Binder.csv"),
                   header = T,
                   sep = ";")

#merge calibrated dates into the previous table
df.final <- merge(neonet, binder, by.x="LabCode", by.y = "Lab_count_id.", all.x = T)

#' add suffixes to duplicated layer names
#'
#' @name neo_leapfrog
#'
#' @description read, format and merge Leapfrog with NeoNet
#'
#' @param layers nodes
#'
...
#'
#' @export
neo_leapfrog <- function(data.neonet = "https://raw.githubusercontent.com/historical-time/caa23/main/neonet/data/140_id00140_doc_elencoc14.csv",
                         data.leapfrog = "https://raw.githubusercontent.com/historical-time/caa23/main/neonet/data/Binder.csv",
                            verbose = T){
  #load NeoNet file
  nn <- read.csv(paste0(file = data.neonet),
                     header = T,
                     sep = ";")

  #load Binder file
  lf <- read.csv(paste0(file = data.leapfrog),
                     header = T,
                     sep = ";")


  #merge calibrated dates into the previous table
  df.final <- merge(nn, lf, by.x="LabCode", by.y = "Lab_count_id.", all.x = T)
